...
elseif($rol==2){
$sql = "select * from estudiante_has_tarea, estudiante, tarea, usuarios where estudiante_id_alumno=id_alumno and tarea_idtarea=idtarea and id_usuario=id_us and usuario='".$_GET['sesion']."'";
$result = $con->prepare($sql);
$result->execute();
$personas = $result->fetchAll(PDO::FETCH_CLASS, "Persona");
}
elseif($rol==3){
$sql = "select * from estudiante_has_tarea, estudiante, tarea, estudiante_has_curso, curso, docente, usuarios where estudiante_has_tarea.estudiante_id_alumno=id_alumno and tarea_idtarea=idtarea and estudiante_has_curso.estudiante_id_alumno=id_alumno and curso_idcurso=idcurso and docente_id_docente=id_docente and id_usuario1=id_us and usuario='".$_GET['sesion']."'";
$result = $con->prepare($sql);
$result->execute();
$personas = $result->fetchAll(PDO::FETCH_CLASS, "Persona");
}
...
<div class="container-fluid">
	...
	if($rol==1||$rol==3){
		?>
		<button><a style="color:green;" href="add_estudiante_has_tareaDB.php?sesion=<?=$sesion?>&rol=<?=$rol?>">Agregar estudiante tiene tarea</a></button>
		<?php
	}
	?>
