...
elseif($rol==3){
$sql = "select * from tarea, materia, curso, docente, usuarios where materia_idmateria1=idmateria and curso_idcurso=idcurso and curso.docente_id_docente=id_docente and id_usuario1=id_us and usuario='".$_GET['sesion']."'";
$result = $con->prepare($sql);
$result->execute();
$personas = $result->fetchAll(PDO::FETCH_CLASS, "Persona");
}
...
<div class="container-fluid">
	...
	if($rol==1||$rol==3){
		?>
		<button><a style="color:green;" href="add_tareaDB.php?sesion=<?=$sesion?>&rol=<?=$rol?>">Agregar tarea</a></button>
		<?php
	}
	...
	if($rol==1||$rol==3){
		?>
		<td><button><a style="color:yellow; background-color:black" href="edit_tareaDB.php?idtarea=<?=$p->idtarea;?>&materia_idmateria1=<?=$p->materia_idmateria1;?>&sesion=<?=$sesion?>&rol=<?=$rol?>">Editar tarea</a></button><br><button><a style="color:red;" href="eliminar_tareaDB.php?idtarea=<?=$p->idtarea;?>&sesion=<?=$sesion?>&rol=<?=$rol?>">Eliminar</a></button></td>
		<?php
	}
	?>
