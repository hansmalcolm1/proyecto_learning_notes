...
elseif($rol==2){
$sql = "select * from curso, docente, estudiante_has_curso, estudiante, usuarios where docente_id_docente=id_docente and curso_idcurso=idcurso and estudiante_id_alumno=id_alumno and id_us=id_usuario and usuario='".$_GET['sesion']."'";
$result = $con->prepare($sql);
$result->execute();
$personas = $result->fetchAll(PDO::FETCH_CLASS, "Persona");
}
elseif($rol==3){
$sql = "select * from curso, docente, usuarios where docente_id_docente=id_docente and id_us=id_usuario1 and usuario='".$_GET['sesion']."'";
$result = $con->prepare($sql);
$result->execute();
$personas = $result->fetchAll(PDO::FETCH_CLASS, "Persona");
}
...
<div class="container-fluid">
	...
	if($rol==1){
		?>
		<button><a style="color:green;" href="add_cursoDB.php?sesion=<?=$sesion?>&rol=<?=$rol?>">Agregar curso</a></button>
		<?php
	}
	if($rol==3){
		?>
		<button><a style="color:green;" href="add_curso_docenteDB.php?sesion=<?=$sesion?>&rol=<?=$rol?>">Agregar curso</a></button>
		<?php
	}
	?>
	...
	if($rol==1){
		?>
		<td><button><a style="color:yellow; background-color:black" href="edit_cursoDB.php?idcurso=<?=$p->idcurso;?>&docente_id_docente=<?=$p->docente_id_docente;?>&sesion=<?=$sesion?>&rol=<?=$rol?>">Editar curso</a></button><br><button><a style="color:red;" href="eliminar_cursoDB.php?idcurso=<?=$p->idcurso;?>&sesion=<?=$sesion?>&rol=<?=$rol?>">Eliminar</a></button></td>
		<?php
	}
	?>
