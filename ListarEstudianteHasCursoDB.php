...
elseif($rol==2){
$sql = "select * from curso, docente, estudiante_has_curso, estudiante, usuarios where docente_id_docente=id_docente and curso_idcurso=idcurso and estudiante_id_alumno=id_alumno and id_usuario=id_us and usuario='".$_GET['sesion']."'";
$result = $con->prepare($sql);
$result->execute();
$personas = $result->fetchAll(PDO::FETCH_CLASS, "Persona");
}
elseif($rol==3){
$sql = "select * from curso, docente, estudiante_has_curso, estudiante, usuarios where docente_id_docente=id_docente and curso_idcurso=idcurso and estudiante_id_alumno=id_alumno and id_usuario1=id_us and usuario='".$_GET['sesion']."'";
$result = $con->prepare($sql);
$result->execute();
$personas = $result->fetchAll(PDO::FETCH_CLASS, "Persona");
}
...
<div class="container-fluid">
	...
	if($rol==1||$rol==3){
		?>
		<button><a style="color:green;" href="add_estudiante_has_cursoDB.php?sesion=<?=$sesion?>&rol=<?=$rol?>">Agregar estudiante has curso</a></button>
		<?php
	}
	?>
	...
	if($rol==1||$rol==3){
		?>
		<td><button><a style="color:yellow; background-color:black" href="edit_estudiante_has_cursoDB.php?estudiante_id_alumno=<?=$p->estudiante_id_alumno;?>&docente_id_docente=<?=$p->docente_id_docente;?>&sesion=<?=$sesion?>&rol=<?=$rol?>">Editar estudiante has curso</a></button><br><button><a style="color:red;" href="eliminar_estudiante_has_cursoDB.php?estudiante_id_alumno=<?=$p->estudiante_id_alumno;?>&curso_idcurso=<?=$p->curso_idcurso;?>&sesion=<?=$sesion?>&rol=<?=$rol?>">Eliminar</a></button></td>
		<?php
	}
	?>
