...
elseif($rol==2){
$sql = "select * from observacion, registro_matricula, estudiante, usuarios where registro_matricula_id=registro_matricula.id and estudiante_id_alumno=id_alumno and id_usuario=id_us and usuario='".$_GET['sesion']."'";
$result = $con->prepare($sql);
$result->execute();
$personas = $result->fetchAll(PDO::FETCH_CLASS, "Persona");
}
elseif($rol==3){
$sql = "select * from observacion, registro_matricula, curso, docente, usuarios where registro_matricula_id=registro_matricula.id and curso_idcurso=idcurso and docente_id_docente=id_docente and id_usuario1=id_us and usuario='".$_GET['sesion']."'";
$result = $con->prepare($sql);
$result->execute();
$personas = $result->fetchAll(PDO::FETCH_CLASS, "Persona");
}
...
<div class="container-fluid">
	...
	if($rol==1||$rol==3){
		?>
		<button><a style="color:green;" href="add_observacionDB.php?sesion=<?=$sesion?>&rol=<?=$rol?>">Agregar observación</a></button>
		<?php
	}
	?>
	...
	if($rol==1||$rol==3){
		?>
		<td><button><a style="color:yellow; background-color:black" href="edit_observacionDB.php?id_observa=<?=$p->id_observa;?>&registro_matricula_id=<?=$p->registro_matricula_id;?>&sesion=<?=$sesion?>&rol=<?=$rol?>">Editar obsevación</a></button><br><button><a style="color:red;" href="eliminar_observacionDB.php?id_observa=<?=$p->id_observa;?>&sesion=<?=$sesion?>&rol=<?=$rol?>">Eliminar</a></button></td>
		<?php
	}
	?>
