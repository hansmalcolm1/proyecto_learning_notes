...
isset($_POST["docente_id_docente"]) && strlen($_POST["docente_id_docente"])>0){
	...
	$sql = "insert into cronograma (actividad, fecha_actividad, docente_id_docente) values (:actividad, :fecha_actividad, :docente_id_docente)";
	...
	echo "<script>alert('La actividad, la fecha de la actividad y el docente son requeridos');
	window.location.href='ListarCronogramasDB.php?sesion=$sesion&rol=$rol'</script>";
}
}
else{
